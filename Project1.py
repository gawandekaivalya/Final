# -*- coding: utf-8 -*-
"""
Created on Tue Feb 23 07:51:10 2016
...
if rangekf == 1:
	...
	alpha = 333.3   # high kf
	beta = 0.2609
	uo = np.array([0.446751,5.275282,0.393890])   #bz2 high kf
	u1 = np.array([0.446751,5.275282,0.393895])
...
if rangekf == 2:
	...
	alpha = 666.7   # high kf
	beta = 0.3478
	uo = np.array([0.0468627,0.89870,0.846515]); u1=np.array([0.0468627,0.89870,0.846525])  # bz2 low kf
...
results1=integrate.odeint(BZ,u1,iters)
...
if plotattractor == 1:
	points3d(np.log10(results[:,0]),np.log10(results[:,1]),np.log10(results[:,2]),iters,color=(0.,0.,0.),scale_factor = pointsize); points3d(np.log10(results1[:,0]),np.log10(results1[:,1]),np.log10(results1[:,2]),iters,color=(1,1,1),scale_factor = pointsize)
